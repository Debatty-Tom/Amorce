...
class FundsTable extends Component
{
    ...
    {
        return DB::table('transaction_summary_view')
            ->whereNull('deleted_at')
            ->where('fund_type', 'principal')
            ->where('fund_title', 'like', '%' . $this->getSearch('principal') . '%')
            ->orderBy($this->getSortField('principal'), $this->getSortDirection('principal'))
            ->paginate(4, pageName: 'principal');
    }
    ...
    public function specificFunds()
    {
        return DB::table('transaction_summary_view')
            ->whereNull('deleted_at')
            ->where('fund_type', 'specific')
            ->where('fund_title', 'like', '%' . $this->getSearch('specific') . '%')
            ->orderBy($this->getSortField('specific'), $this->getSortDirection('specific'))
            ->paginate(4, pageName: 'specific');
    }
    ...
    public function archivedFunds()
    {
        return DB::table('transaction_summary_view')
            ...
            ->where('fund_title', 'like', '%' . $this->getSearch('archived') . '%')
            ->orderBy($this->getSortField('archived'), $this->getSortDirection('archived'))
            ->paginate(4, pageName: 'archived');
    }
    #[on('refresh-funds')]
